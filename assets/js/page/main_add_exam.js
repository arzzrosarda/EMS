...
$(document).ready(function (){
    ...
    $("#depselect").on('change', function (){
        var department = $("#depselect option:selected").val();
        if (department != ''){
            $.post("fragments/division_select.php", {department:department}, function (div){
                $("#divselect").html(div);
            });
        }else {
            $.post("fragments/disabled_division.php", function (div){
...
$("#btnsbmtexamtitle").click( function (){
    ...
    if (title == ''){
        ...
        iziToast.warning({
            title: 'Empty!',
            message: 'Department for exam is required',
            position: 'topRight'
        });
    }else if (division == ''){
        $('#divselect').focus();
