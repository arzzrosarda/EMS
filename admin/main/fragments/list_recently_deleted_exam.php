...
<table class="table table-sm" id="Tlog">
    ...
    if (isset($_REQUEST['division']) && isset($_REQUEST['department']) && isset($_REQUEST['status'])){
        ...
        $department = $_REQUEST['department'];
        $status = $_REQUEST['status'];
        $examQuery2 = $conn->query("SELECT * FROM exam_title WHERE department = '$department' AND isActiveExam = '$status' AND isDeletedExam = '1'");
    }
        ...
        $department = $_REQUEST['department'];
        $examQuery2 = $conn->query("SELECT * FROM exam_title WHERE department = '$department' AND isDeletedExam = '1'");
    }
    else {
        $examQuery2 = $conn->query("SELECT * FROM exam_title WHERE isDeletedExam = '1'");
    }
