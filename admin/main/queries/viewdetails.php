...
if (isset($_REQUEST['userid'])){
    ...
    $result1 .= '<script>
                    $(document).ready( function (){
                        ...
                        if (dep != ""){
                            $.post("fragments/division_label_select.php", {department:dep, user_id:'. $userid .'}, function (div){
                                $("#divisionsel").html(div);
                            });
                        }else {
                            $.post("fragments/division_label_select.php", function (div){
                                $("#divisionsel").html(div);
                            });
                        }
                        $("#deptselect").on("change", function (){
                            var department = $("#deptselect option:selected").val();
                            if (department != ""){
                                $.post("fragments/division_label_select.php", {department:department, user_id:'. $userid .'}, function (div){
                                    $("#divisionsel").html(div);
                                });
                            }else {
                                $.post("fragments/division_label_select.php", function (div){
    ...
    $result1 .= '<br><table class="table table-sm table-striped table-responsive-lg">
                   <thead>
                   ...
                   <tbody>
                    ...
                    <tr style="line-height: 25px;">
                        <td>Department: </td>
                        <td id="td_department">' . $user['department'] . ' </td>
                        <td id="td_department_edit" style="display: none;">
                            <select id="deptselect" name="department" class="form-control">
                                <option value="">Select Department</option>';
                                $department_query = $conn->query("SELECT * FROM department");
                                while ($department = $department_query->fetch()){
                                    $result1 .= '<option value="'.$department['department']. '">'. $department['department'].'</option>';
                                }
                $result1 .= '</select>
                        </td>
                    </tr>
