    ...
    if ($Uquery->rowCount() == 0){
        if ($Nquery->rowCount() == 0){
            if ($Username_query->rowCount() == 0){
                $inquery = $conn->query("UPDATE user SET exam_no = '$examno', username = '$username', lname = '$lnam', fname = '$fnam', mname = '$mnam', gender = '$gender', email = '$email', home_address = '$haddress', brgy = '$brgy', city = '$citymun', province = '$prov', postal_code = '$post', contact_no = '$MobNo' WHERE id = '$user_id'");
                $result = 1;
                if ($inquery){
